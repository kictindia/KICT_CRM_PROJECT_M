...
const PayFee = () => {
  ...
  //           student.FeeSlot.map((feeSlot, index) => ({
  ...
  useEffect(() => {
    const parseDate = (dateStr) => {
      if (dateStr) {
        const [day, month, year] = dateStr.split(" "); // Split by space for DD MM YYYY format
        return new Date(year, month - 1, day); // JavaScript months are 0-based
      }
      return null;
    };
  ...
  const options = {
    ...
    onDownload: (buildHead, buildBody, columns, data) => {
      ...
      const pageName = "PayFee"; // Use the page name as a constant
      const today = new Date();
