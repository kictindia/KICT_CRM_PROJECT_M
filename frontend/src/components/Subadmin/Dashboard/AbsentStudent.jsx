    ...
    const fetchAbsentStudents = async () => {
      try {
        ...
        const absentStudentsList = attendanceData
          .filter(entry => entry.Date === today && entry.Course === franchiseId) // Ensure matching Course (FranchiseId) and Date
          .flatMap(entry =>
            entry.Attendance
              .filter(student => student.Status === "Absent") // Filter only absent students
              .map(student => ({
                name: student.Name,
        ...
        const studentsWithImages = await Promise.all(
          absentStudentsList.map(async (student) => {
            try {
              const studentResponse = await fetch(`https://franchiseapi.kictindia.com/student/get/${student.studentId}`);
